...
module.exports = function(app) {
  ...
  utility.checkGender = function(value) {
    return isFinite(value) ?
      Object.keys(app.config.user.gender)
      .map((each) => app.config.user.gender[each])
  ...
  utility.getNanoId = function(length) {
    return nanoId
      .customAlphabet('0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ', length || 10)()
      .toUpperCase();
  };
  ...
  utility.format = {
    user: function(userObj) {
      var newUserObj = JSON.parse(JSON.stringify(userObj));
  ...
  utility.isValidate = {
    ...
    checkAllConsecutivesTimeSlots: (arr) => {
      return arr.every((each, index) => {
        if (each === 48 && arr.length - 1 !== index) {
          return (arr[index + 1] || each + 1) - each === -47;
        } else {
          return (arr[index + 1] || each + 1) - each === 1;
        }
      });
    },
  };

  utility.getTimeString = (startingSlot) => {
    let amOrPM = 'AM';
    let startHour = Math.floor((startingSlot - 1) / 2);
    if (startHour === 12) {
      amOrPM = 'PM';
    ...
    let startMinute = Math.floor((startingSlot - 1) % 2) === 1 ? '30' : '00';
    return `${startHour === 0 ? 12 : startHour}:${startMinute} ${amOrPM}`;
  };

  utility.updateActivity = (userDoc, activityRefs, isActivity, activityRef) => {
    let updatedAcivityRefs = [];
    if (isActivity) {
      if (
        !userDoc[activityRefs] ||
        (userDoc[activityRefs] &&
          Array.isArray(userDoc[activityRefs]) &&
          userDoc[activityRefs].length === 0)
      ) {
        userDoc[activityRefs] = [activityRef];
      } else {
        updatedAcivityRefs = JSON.parse(JSON.stringify(userDoc[activityRefs]));
        let indexOfActivityRef = updatedAcivityRefs.indexOf(activityRef);
        if (indexOfActivityRef === -1) {
          updatedAcivityRefs = [...updatedAcivityRefs, activityRef];
          userDoc[activityRefs] = [...updatedAcivityRefs];
        }
      }
    } else {
      if (
        userDoc[activityRefs] &&
        Array.isArray(userDoc[activityRefs]) &&
        userDoc[activityRefs].length
      ) {
        updatedAcivityRefs = JSON.parse(JSON.stringify(userDoc[activityRefs]));
        let indexOfActivityRef = updatedAcivityRefs.indexOf(activityRef);
        if (indexOfActivityRef !== -1) {
          updatedAcivityRefs.splice(indexOfActivityRef, 1);
          userDoc[activityRefs] = [...updatedAcivityRefs];
        }
      }
    }
